...
from methods.tot.base import Node
...
class ToT_Task(SearchTask):
    def __init__(self, data, propose_method='glm', value_method='glm', algorithm='dfs', branch=1, select_branch=1,
                 max_depth=1, end_gate=0.9, select_method='greedy',
                 temperature=0.7, max_tokens=2048,
                 seed=170, max_length=2048, truncation=True,
                 do_sample=True, max_new_tokens=256, use_case_prompt=False, low=0, high=1, evaluate='', multiply_value=False, lang='en', answer=None, verify_method='string'):
    ...
    def get_next_step(self, y, step_n):
        ...
        response = self.propose_method.generate(prompt, max_new_tokens=self.max_tokens).split('\n')
        # print(f'response:\n{response}')
        if not response:
    ...
    def get_step_value(self, y):
        ...
        if self.value_method == 'local':
            ...
            value = self.value_method.generate(prompt_answer, max_new_tokens=self.max_length)
            ...
            response = self.value_method.generate(prompt, max_new_tokens=self.max_length)
    ...
    def get_summary(self, y):
        if self.lang == 'zh':
            ...
            response = self.propose_method.generate(prompt, max_new_tokens=128)
